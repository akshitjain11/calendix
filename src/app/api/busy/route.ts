...
export async function GET(req:NextRequest) {
    ...
    if (!profileDoc) {
        return Response.json('invalid username and/or Booking Uri',{status:404})
    }

    const nylasBusyResult = nylas.calendars.getFreeBusy({
    ...
    if ((await nylasBusyResult).data?.[0]) {
        //@ts-ignore
        const slots = (await nylasBusyResult).data?.[0]?.timeSlots as TimeSlot[];
         //@ts-ignore
